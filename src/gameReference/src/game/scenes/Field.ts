...
export class Field extends BaseGameScene {
  constructor() {
    super({ key: "field" });
  }
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("field-map", "assets/FieldMap.tmj");
  ...
  async create() {
    ...
    this.map = this.make.tilemap({ key: "field-map" });
    ...
    this.room.state.spawnedPlayers.onAdd((player) => {
      ...
      this.anims.create({
        key: "white-boar-idle",
        frames: this.anims.generateFrameNumbers("white-boar-idle", {
          start: 0,
          end: 3,
        }),
        frameRate: 8,
      ...
      this.anims.create({
        key: "white-boar-walk",
        frames: this.anims.generateFrameNumbers("white-boar-walk", {
          start: 0,
          end: 5,
        }),
        frameRate: 8,
      ...
      this.anims.create({
        key: "white-boar-run",
        frames: this.anims.generateFrameNumbers("white-boar-run", {
          start: 0,
          end: 5,
        }),
        frameRate: 8,
      ...
      this.anims.create({
        key: "white-boar-vanish",
        frames: this.anims.generateFrameNumbers("white-boar-vanish", {
          start: 0,
          end: 3,
        }),
        frameRate: 12,
    ...
    this.room.state.spawnedMonsters.onAdd((monster, index) => {
      const entity = this.physics.add.sprite(
        monster.x,
        monster.y,
        monster.name === "White Boar" ? "white-boar-idle" : "boar-idle"
      );
      ...
      monster.onChange((changes) => {
        ...
        if (monster.currentHealth <= 0) {
          ...
          entity.play(
            monster.name === "Boar" ? "boar-vanish" : "white-boar-vanish",
            true
          );
          ...
          // Monster took damage but didn't die - play vanish animation
          entity.play(
            monster.name === "Boar" ? "boar-vanish" : "white-boar-vanish",
            true
          );
  ...
  private updateMonsterAnimation(entity, monster) {
    ...
    // Set the appropriate animation based on behavior state
    switch (behaviorState) {
      case "run":
        entity.play(
          monsterName === "Boar" ? "boar-run" : "white-boar-run",
          true
        );
        break;
      case "walk":
        entity.play(
          monsterName === "Boar" ? "boar-walk" : "white-boar-walk",
          true
        );
        break;
      case "idle":
      default:
        entity.play(
          monsterName === "Boar" ? "boar-idle" : "white-boar-idle",
          true
        );
        break;
    }

    // Ensure correct size is maintained after animation changes
    if (monsterName === "Boar") {
      entity.setDisplaySize(48, 32);
    } else if (monsterName === "White Boar") {
      entity.setDisplaySize(48, 32);
    }
  }
}
